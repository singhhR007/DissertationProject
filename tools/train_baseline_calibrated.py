...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Train a Platt-calibrated HDFS baseline. "
            "Trains LogisticRegression on a train split, then applies "
            "CalibratedClassifierCV(method='sigmoid', cv='prefit') on a "
    ...
    parser.add_argument(
        "--output-dir",
        default="artefacts/models/hdfs_baseline_calibrated",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.05,
        help="Fraction of the dataset reserved for the calibration split.",
    )
    parser.add_argument(
        "--val-size",
        type=float,
        default=0.05,
        help="Fraction of the dataset reserved for the validation split.",
    )
    parser.add_argument(
        "--random-state",
        type=int,
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "A stratified subset is sampled before splitting."
        ),
    )
...
def main() -> None:
    ...
    if args.test_size <= 0 or args.calibration_size <= 0 or args.val_size <= 0:
        raise ValueError("test_size, calibration_size, and val_size must all be > 0.")

    if args.test_size + args.calibration_size + args.val_size >= 1.0:
        raise ValueError(
            "test_size + calibration_size + val_size must be < 1.0."
        )
    ...
    # Four-way stratified split: train (70%), calibration (5%), val (5%), test (20%).
    # Performed in two steps. First peel off the test split. Then split the remainder
    # into train, calibration, and validation using relative sizes.
    all_indices = np.arange(total_count)

    train_cal_val_idx, test_idx = train_test_split(
        all_indices,
    ...
    remaining_fraction = 1.0 - args.test_size
    calibration_relative = args.calibration_size / remaining_fraction
    val_relative = args.val_size / remaining_fraction

    train_cal_idx, val_idx = train_test_split(
        train_cal_val_idx,
        test_size=val_relative,
        random_state=args.random_state,
        stratify=labels[train_cal_val_idx],
    )

    # Convert calibration size to a fraction of the remaining (train + calibration) pool.
    cal_relative_after_val = calibration_relative / (1.0 - val_relative)

    train_idx, calibration_idx = train_test_split(
        train_cal_idx,
        test_size=cal_relative_after_val,
        random_state=args.random_state,
        stratify=labels[train_cal_idx],
    )

    train_idx = [int(i) for i in train_idx]
    calibration_idx = [int(i) for i in calibration_idx]
    val_idx = [int(i) for i in val_idx]
    test_idx = [int(i) for i in test_idx]

    train_texts: list[str] = [texts[i] for i in train_idx]
    calibration_texts: list[str] = [texts[i] for i in calibration_idx]
    val_texts: list[str] = [texts[i] for i in val_idx]
    test_texts: list[str] = [texts[i] for i in test_idx]

    y_train = labels[train_idx]
    y_calibration = labels[calibration_idx]
    y_val = labels[val_idx]
    y_test = labels[test_idx]

    print()
    print("Split sizes:")
    print(f"  train:       {len(train_idx)} (anomalous: {int(y_train.sum())})")
    print(f"  calibration: {len(calibration_idx)} (anomalous: {int(y_calibration.sum())})")
    print(f"  validation:  {len(val_idx)} (anomalous: {int(y_val.sum())})")
    print(f"  test:        {len(test_idx)} (anomalous: {int(y_test.sum())})")

    save_split_manifest(output_dir, "train", [sequence_ids[i] for i in train_idx])
    save_split_manifest(output_dir, "calibration", [sequence_ids[i] for i in calibration_idx])
    ...
    x_calibration = vectorizer.transform(calibration_texts)
    x_val = vectorizer.transform(val_texts)
    x_test = vectorizer.transform(test_texts)

    print("Training base Logistic Regression on the train split...")
    ...
    print("Calibrating with Platt scaling on the calibration split...")
    ...
    # Persist the calibrated pipeline. The deployed loader expects vectorizer +
    # the calibrated estimator under the 'classifier' key, plus calibration metadata.
    artefact: dict[str, Any] = {
        "model_type": "tfidf_logistic_regression_platt_calibrated",
    ...
    report: dict[str, Any] = {
        "dataset": "HDFS",
        "model_type": "TF-IDF + Logistic Regression + Platt scaling",
        "text_mode": args.text_mode,
        "paths": {
            "hdfs_log": str(args.hdfs_log),
            "hdfs_labels": str(args.hdfs_labels),
            "model_artefact": str(model_path),
        },
        ...
        "split_summary": {
            ...
            "calibration_anomalous": int(y_calibration.sum()),
    ...
    print(f"Saved model artefact: {model_path}")
    ...
    if test_tuned["roc_auc"] is not None:
        ...
        print("  ROC-AUC:     n/a")
    print(f"  Brier score: {test_tuned['brier_score']:.6f}")
